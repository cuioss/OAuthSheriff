...
* **JMH vs integration gap**: Core library validates in 77µs (JMH isolation), but integration validation costs 1.087ms — 14.1x more. Estimated split: JWT validation under concurrency ~600-800µs (concurrency multiplier, real object allocation) + CDI `Instance.get()` overhead ~160-325µs (CreationalContext, InjectionPoint introspection, `@Timed` instrumentation). This split is estimated from JFR data, not measured via ablation.
* **Dominant cost**: CDI producer chain combined with JWT validation accounts for 57% (1.087ms) of request latency. Within this layer, the concurrency penalty on `createAccessToken()` is the estimated primary contributor (~55-74%), with CDI `Instance.get()` machinery secondary (~15-30%). JAX-RS POST routing adds 14% (0.274ms); Vert.x context resolution and header parsing are negligible (~2% combined)
...
After systematically ruling out lock contention, GC, access log filter, CDI `@ApplicationScoped` injection, and `@RequestScoped` refactoring, the 1.087ms cost has been decomposed into two contributing factors. The ablation study measures the combined cost as a single layer (JWT minus Mock JWT) — the breakdown below is **estimated from JFR data**, not measured via ablation.

==== Contributing Factor 1: JWT Validation Under Concurrency (~600-800µs, estimated 55-74%)

The core JWT library validates in 77µs (JMH, single-thread isolation). Under 50 concurrent connections, this cost increases by an estimated 8-10x due to:
...
==== Contributing Factor 2: CDI Instance.get() Overhead (~160-325µs, estimated 15-30%)

Each `Instance.get()` call in Quarkus ArC performs:

1. **Child CreationalContext allocation** — `this.creationalContext.child(bean)` (~100-200µs; 250 JFR allocation samples)
2. **InjectionPointImpl construction** — type, qualifiers, annotations, member info (~50-100µs)
...
5. **`@Timed` Micrometer instrumentation** — metrics recording per validation (~1-5µs)
...
==== Decomposition Gap: Unmeasured Split

The existing ablation study measures the 1.087ms as a single layer. The split between CDI `Instance.get()` overhead (~160-325µs) and `createAccessToken()` under concurrency (~600-800µs) is **estimated from JFR allocation data**, not measured. An additional ablation variant that calls `tokenValidator.createAccessToken()` directly (bypassing `Instance.get()`) would provide a measured split — the existing `/jwt/validate-explicit` endpoint is a candidate but was not included in the benchmark suite.
...
| ⚠️
| Estimated from JFR data, not measured via ablation
...
| Low
| An additional ablation variant could measure the split, but neither factor is optimizable
|===
