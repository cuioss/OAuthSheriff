...
**Conclusion:** The core library validation is highly optimized at 159k ops/s with 77µs latency. Integration testing at 50 connections shows 22.7k ops/s and 1.93ms P50 latency. A six-layer ablation study (link:Analysis-01.2026-Latency-Decomposition.adoc[Latency Decomposition]) decomposes the 1.85ms overhead into: shared infrastructure 0.510ms (27%), JAX-RS POST routing 0.274ms (14%), Vert.x context resolution 0.036ms (2%), authorization parsing 0.003ms (<1%), and the formerly combined "CDI producer chain + JWT validation" layer now split via a direct-validation ablation variant into JWT validation under concurrency 1,160µs (55.8%) and CDI `Instance.get()` + `@Timed` overhead ~50µs (2.4%). The combined 1,210µs is close to the original 1,087µs — the difference is measurement variance between runs.
...
**Key Takeaway:** The core library is extremely fast (77µs P50). Integration overhead at 50 connections adds ~1.85ms (measured decomposition in link:Analysis-01.2026-Latency-Decomposition.adoc[Latency Decomposition]), of which JWT validation under concurrency accounts for 1,160µs (55.8%, measured) and CDI overhead only 50µs (2.4%, measured). This is acceptable for real-world HTTP-based microservices.
...
* **Integration reality check**: 7x throughput gap (159k vs 23k ops/s) and 25x latency gap (77µs vs 1.93ms at 50 connections) — integration overhead decomposed via ablation study with measured CDI vs validation split: JWT validation 1,160µs (55.8%), CDI overhead 50µs (2.4%) (link:Analysis-01.2026-Latency-Decomposition.adoc[Latency Decomposition])
