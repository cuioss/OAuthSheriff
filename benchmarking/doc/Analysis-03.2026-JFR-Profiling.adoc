...
NOTE: Despite 250 `CreationalContextImpl` allocations recorded in JFR, the measured CDI overhead via ablation is only 50µs (2.4%). The allocations are fast individually, and the real cost lies in JWT validation under concurrency (1,160µs, 55.8%).
...
1. **Two contributing factors identified**: JWT validation under concurrency at 55.8% (1,160µs), and CDI `Instance.get()` with per-request `InjectionPoint` introspection at 2.4% (50µs)
...
3. **Split is measured**: JWT validation under concurrency dominates at 1,160µs, while CDI overhead is minimal at 50µs
