...
The ~14x gap between JMH (77µs) and integration JWT validation under concurrency (1,060µs, measured via ablation) cannot be explained by CPU time alone. JFR shows CUI library at only 0.3% of CPU samples, yet latency decomposition shows JWT validation at 46% of total request time. This section analyzes non-CPU overhead.
...
**Does GC explain the ~14x gap?** No. At 22 GCs/second and ~25,000 requests/second, only ~1 in 1,140 requests encounters a GC pause. The 3.7% overhead is amortized — most individual requests complete without GC interference. GC contributes to tail latency (P99) but is not the primary cause of the 1.06ms token validation cost under concurrency.
...
The JFR data reveals the ~14x gap is primarily due to concurrency effects, not CDI overhead (which is only 20µs per the ablation study):

1. **Object allocation under concurrency** — Each validation creates new HashMaps, Sets, `AccessTokenContent`, and result objects (vs JMH reusing pre-parsed data)
2. **Memory barriers** — Lock-free cache (`ConcurrentHashMap`) operations require memory ordering guarantees
3. **Thread scheduling** — 50 concurrent connections cause frequent context switches (not visible in JFR as individual events)
4. **CPU cache line invalidation** — Concurrent access to shared data structures invalidates L1/L2 cache
5. **CDI `Instance.get()` overhead** — `CreationalContext` creation and `InjectionPoint` introspection add only 20µs (0.9%, measured via ablation)
...
NOTE: Despite 250 `CreationalContextImpl` allocations recorded in JFR, the measured CDI overhead via ablation is only 20µs (0.9%). The allocations are fast individually, and the real cost lies in JWT validation under concurrency (1,060µs, 46.1%).
...
The ~14x JMH-to-integration gap (77µs → 1,060µs under concurrency) is **understood but not optimizable**:

1. **Two contributing factors identified**: JWT validation under concurrency at 46.1% of total request time (1,060µs), and CDI `Instance.get()` with per-request `InjectionPoint` introspection at 0.9% (20µs)
...
3. **Split is measured**: JWT validation under concurrency dominates at 1,060µs, while CDI overhead is minimal at 20µs
4. **Performance acceptable**: 25.2K ops/s at 2.30ms avg exceeds targets (>10K ops/s, <5ms)

== Related Documents

* link:Analysis-03.2026-Integration.adoc[Integration Benchmark Analysis] - Throughput and latency measurements
* link:Analysis-03.2026-Latency-Decomposition.adoc[Latency Decomposition] - Ablation study results
* link:Analysis-03.2026-Micro.adoc[Microbenchmark Analysis] - JMH component-level timings
