...
NOTE: Despite 250 `CreationalContextImpl` allocations recorded in JFR, the measured CDI overhead via ablation is only 50µs (2.4%). The allocation count overestimated the actual latency impact — the allocations are fast individually, and the real cost lies in JWT validation under concurrency (1,160µs, 55.8%).
...
NOTE: The ablation study with the direct-validation variant now provides a measured split. The estimates below are retained for reference but superseded by measured values: JWT validation under concurrency 1,160µs (55.8%), CDI `Instance.get()` + `@Timed` ~50µs (2.4%).
...
1. **Two contributing factors identified**: JWT validation under concurrency at 55.8%, measured (1,160µs), and CDI `Instance.get()` with per-request `InjectionPoint` introspection at 2.4%, measured (50µs)
...
3. **Split is measured**: The direct-validation ablation variant (bypassing `Instance.get()`) confirms JWT validation under concurrency dominates (1,160µs), while CDI overhead is minimal (50µs)
