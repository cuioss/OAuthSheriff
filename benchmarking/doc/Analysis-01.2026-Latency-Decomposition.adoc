...
|**6. CDI producer chain + JWT validation** (`Instance.get()`, producer resolution, `createAccessToken()`, claims checking)
|1.087ms
|56.9%
|JWT minus Mock JWT (1.91 − 0.823) — combined measurement, see note below
|===
...
**CDI producer chain + JWT validation remains the dominant cost at 57%.** The 1.087ms gap between JWT and mock JWT (consistent with the previous measurement of 1.117ms) is a **combined measurement** of two factors: (1) `tokenValidator.createAccessToken()` running under concurrency (~600-800µs estimated, 8-10x slower than the 77µs JMH baseline due to cache contention, memory barriers, and real object allocation), and (2) CDI `Instance.get()` overhead (~160-325µs estimated, including `CreationalContext` allocation, `InjectionPoint` introspection, and `@Timed` instrumentation). The existing ablation study does not separate these two factors — an additional variant calling `createAccessToken()` directly (bypassing `Instance.get()`) would provide a measured split.
...
The JMH micro-benchmark measures isolated JWT library validation at 0.077ms (77µs). The mock endpoint experiment reveals the actual cost difference between JWT and mock is 1.087ms — **14.1x more** than the isolated library. This gap is a combined measurement of two contributing factors:

**Primary factor (~55-74%, estimated): JWT validation under concurrency**
...
* Estimated at ~600-800µs (8-10x concurrency multiplier over 77µs JMH baseline)

**Secondary factor (~15-30%, estimated): CDI Instance.get() overhead**
...
* Estimated at ~160-325µs (from JFR allocation data: 250 `CreationalContextImpl` samples)

NOTE: This split is estimated from JFR data. The ablation study measures the combined 1.087ms but does not separate CDI overhead from validation concurrency cost. An additional ablation variant calling `createAccessToken()` directly would provide a measured split.
