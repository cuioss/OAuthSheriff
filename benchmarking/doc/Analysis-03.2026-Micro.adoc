...
* **Library Performance**: ~6x faster than integration tests (159k vs 25k ops/s)
...
**Observation:** Library micro-benchmarks achieve 159k ops/s while integration tests (WRK) show 25k ops/s.
...
|25,178 ops/s, 2.30ms avg latency

|Throughput gap
|~6x slower (134k ops/s difference)

|Latency gap
|~30x slower (2.22ms overhead, comparing JMH P50 vs WRK avg)

|Integration overhead breakdown
|Measured via ablation study — see link:Analysis-03.2026-Latency-Decomposition.adoc[Latency Decomposition]
|===

NOTE: At higher connection counts, latency increases due to connection queuing (e.g., 300 connections: 15.11ms avg). The 50-connection baseline is used here for a fair comparison with JMH, which runs without queuing effects.

**Conclusion:** The core library validation is highly optimized at 159k ops/s with 77µs latency. Integration testing at 50 connections shows 25.2k ops/s and 2.30ms avg latency. A six-layer ablation study (link:Analysis-03.2026-Latency-Decomposition.adoc[Latency Decomposition]) decomposes the 2.30ms into: shared infrastructure 0.78ms (33.9%), JAX-RS POST routing 0.40ms (17.4%), Vert.x context resolution 0.06ms (2.6%), authorization parsing ~0ms, JWT validation under concurrency 1.06ms (46.1%), and CDI `Instance.get()` + `@Timed` overhead 0.02ms (0.9%).
...
|Environment
|In-memory JMH (100 threads)
|HTTP/Docker/Quarkus (50 conns)
|Network + framework overhead
|===

NOTE: The 50-connection baseline is used because JMH runs without connection queuing, making it a fair comparison. JMH P50 (77µs) represents single-operation latency; integration avg (2.30ms) includes all HTTP/framework overhead. At 300 connections, JWT avg rises to 15.11ms due to connection queuing, not additional per-request overhead.

**Key Takeaway:** The core library is extremely fast (77µs P50). Integration overhead at 50 connections adds ~2.22ms (measured decomposition in link:Analysis-03.2026-Latency-Decomposition.adoc[Latency Decomposition]), of which JWT validation under concurrency accounts for 1,060µs (46.1%, measured) and CDI overhead only 20µs (0.9%, measured). This is acceptable for real-world HTTP-based microservices.
...
* **Integration reality check**: ~6x throughput gap (159k vs 25k ops/s) at 50 connections — integration overhead decomposed via ablation study with measured CDI vs validation split: JWT validation 1,060µs (46.1%), CDI overhead 20µs (0.9%) (link:Analysis-03.2026-Latency-Decomposition.adoc[Latency Decomposition])
