...
test.describe('02 - JWKS Endpoints Card', () => {
  ...
  test('should display the JWKS endpoints container', async ({ page }) => {
    const container = page.locator(CONSTANTS.SELECTORS.JWKS_ENDPOINTS_CONTAINER);
  ...
  test('should show CONFIGURED status when issuers exist', async ({ page }) => {
    const container = page.locator(CONSTANTS.SELECTORS.JWKS_ENDPOINTS_CONTAINER);
    await container.waitFor({ state: 'visible', timeout: CONSTANTS.TIMEOUTS.ELEMENT_VISIBLE }).catch(() => {});

    if (await container.isVisible()) {
      const statusMessage = page.locator(CONSTANTS.SELECTORS.JWKS_STATUS_MESSAGE);
  ...
  test('should list configured issuers', async ({ page }) => {
    ...
    if (await container.isVisible()) {
      const issuerCards = page.locator(CONSTANTS.SELECTORS.JWKS_ISSUER_CARD);
  ...
  test('should display issuer details with URIs', async ({ page }) => {
    ...
    if (await container.isVisible()) {
      const firstIssuer = page.locator(CONSTANTS.SELECTORS.JWKS_ISSUER_CARD).first();
  ...
  test('should have a working refresh button', async ({ page }) => {
    ...
    if (await container.isVisible()) {
      const refreshButton = page.locator(CONSTANTS.SELECTORS.JWKS_REFRESH_BUTTON);
      ...
      await expect(container.or(page.locator(CONSTANTS.SELECTORS.JWKS_LOADING))).toBeVisible();
    }
  });
});
