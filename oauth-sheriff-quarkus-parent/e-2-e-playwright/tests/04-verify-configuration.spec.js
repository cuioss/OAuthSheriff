...
test.describe('04 - Configuration Card', () => {
  ...
  test('should display the configuration container', async ({ page }) => {
    const container = page.locator(CONSTANTS.SELECTORS.JWT_CONFIG_CONTAINER);
  ...
  test('should show runtime config (not build-time placeholders)', async ({ page }) => {
  ...
  test('should display general settings section', async ({ page }) => {
    const container = page.locator(CONSTANTS.SELECTORS.JWT_CONFIG_CONTAINER);
    await container.waitFor({ state: 'visible', timeout: CONSTANTS.TIMEOUTS.ELEMENT_VISIBLE }).catch(() => {});

    if (await container.isVisible()) {
      const generalSection = page.locator(CONSTANTS.SELECTORS.JWT_CONFIG_GENERAL_SECTION);
  ...
  test('should display parser configuration section', async ({ page }) => {
    ...
    if (await container.isVisible()) {
      const parserSection = page.locator(CONSTANTS.SELECTORS.JWT_CONFIG_PARSER_SECTION);
  ...
  test('should display issuers configuration section', async ({ page }) => {
    ...
    if (await container.isVisible()) {
      const issuersSection = page.locator(CONSTANTS.SELECTORS.JWT_CONFIG_ISSUERS_SECTION);
  ...
  test('should display health indicator', async ({ page }) => {
    ...
    if (await container.isVisible()) {
      const healthIndicator = page.locator(CONSTANTS.SELECTORS.JWT_CONFIG_HEALTH_INDICATOR);
  ...
  test('should have a working refresh button', async ({ page }) => {
    ...
    if (await container.isVisible()) {
      const refreshButton = page.locator(CONSTANTS.SELECTORS.JWT_CONFIG_REFRESH_BUTTON);
      ...
      // Container or loading should still be visible
      await expect(container.or(page.locator(CONSTANTS.SELECTORS.JWT_CONFIG_LOADING))).toBeVisible();
    }
  });
});
