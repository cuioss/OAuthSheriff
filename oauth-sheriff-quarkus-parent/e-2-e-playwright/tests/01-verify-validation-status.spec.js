...
test.describe('01 - JWT Validation Status Card', () => {
  ...
  test('should show RUNTIME data (not BUILD_TIME placeholders)', async ({ page }) => {
  ...
  test('should display enabled/disabled metric', async ({ page }) => {
    ...
    await card.waitFor({ state: 'visible', timeout: CONSTANTS.TIMEOUTS.ELEMENT_VISIBLE }).catch(() => {});

    if (await card.isVisible()) {
      const enabledMetric = page.locator(CONSTANTS.SELECTORS.METRIC_ENABLED);
  ...
  test('should display validator present metric', async ({ page }) => {
    ...
    if (await card.isVisible()) {
      const validatorMetric = page.locator(CONSTANTS.SELECTORS.METRIC_VALIDATOR_PRESENT);
  ...
  test('should display overall status metric', async ({ page }) => {
    ...
    if (await card.isVisible()) {
      const statusMetric = page.locator(CONSTANTS.SELECTORS.METRIC_OVERALL_STATUS);
  ...
  test('should have a working refresh button', async ({ page }) => {
    ...
    if (await card.isVisible()) {
      const refreshButton = page.locator(CONSTANTS.SELECTORS.VALIDATION_REFRESH_BUTTON);
      ...
      await expect(card.or(page.locator(CONSTANTS.SELECTORS.VALIDATION_LOADING))).toBeVisible();
    }
  });
  ...
  test('should display status indicator', async ({ page }) => {
    ...
    if (await card.isVisible()) {
      const indicator = page.locator(CONSTANTS.SELECTORS.VALIDATION_STATUS_INDICATOR);
  ...
  test('should display status message', async ({ page }) => {
    ...
    if (await card.isVisible()) {
      const message = page.locator(CONSTANTS.SELECTORS.VALIDATION_STATUS_MESSAGE);
