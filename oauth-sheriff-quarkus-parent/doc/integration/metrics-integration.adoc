...
* Registers Micrometer timers for each validation pipeline step from the `TokenValidatorMonitor`
...
rate(sheriff_oauth_validation_errors_total{event_type="TOKEN_EXPIRED"}[5m])
...
rate(sheriff_oauth_validation_duration_seconds_sum{step="jwks_operations"}[5m]) / rate(sheriff_oauth_validation_duration_seconds_count{step="jwks_operations"}[5m])
----
...
- alert: JwtHighErrorRate
  expr: rate(sheriff_oauth_validation_errors_total[5m]) > 5
  for: 10m
  labels:
    severity: warning
  annotations:
