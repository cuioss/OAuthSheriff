...
....
              ...
              │   IssuerConfigResolver → HeaderValidator │
...
|RSA-OAEP-256
|`Cipher("RSA/ECB/OAEPPadding")` + `OAEPParameterSpec(SHA-256, MGF1, SHA256, DEFAULT)`
...
|===
|Feature |JJWT (0.12+) |Nimbus JOSE+JWT |Spring Security |jose4j
...
Spring Security's `NimbusJwtDecoder` is JWS-only. JWE requires manual Nimbus `DefaultJWTProcessor` configuration (open since 2017).
