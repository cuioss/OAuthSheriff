...
* EdDSA (Edwards-curve Digital Signature Algorithm using Ed25519/Ed448, https://datatracker.ietf.org/doc/html/rfc8037[RFC 8037])
...
1. Support for multiple signature algorithms (RSA, ECDSA, RSA-PSS, EdDSA)
...
** Parsing OKP (Octet Key Pair) keys for EdDSA via `EdECPublicKeySpec` (JEP 339)
...
** Supporting all RFC 7518 and RFC 8037 signature algorithms:
*** RSA signatures (RS256, RS384, RS512)
*** ECDSA signatures (ES256, ES384, ES512)
*** RSA-PSS signatures (PS256, PS384, PS512)
*** EdDSA signatures (Ed25519, Ed448) via `Signature.getInstance("EdDSA")`
...
* **EdDSA algorithms**: Full support for Ed25519 and Ed448 via JEP 339 (Java 15+), no additional dependencies required
