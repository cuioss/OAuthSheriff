...
* **EventCategory** - `InvalidStructure`, `InvalidSignature`, or `SemanticIssues`
...
`HttpWellKnownResolver` fetches `/.well-known/openid-configuration` documents. The `issuer` field from the discovery document is validated against the URL used to construct the well-known URI, preventing impersonation attacks. The discovered `jwks_uri` is used by `HttpJwksLoader` to fetch public keys.
...
|`maxStringSize` |4 KB |Individual JSON string values
|`maxArraySize` |64 |JSON array element count
|`maxDepth` |10 |JSON nesting depth
|JWKS content |64 KB |Default upper bound for JWKS payloads
|===

Conservative (16 KB) and permissive (256 KB) JWKS bounds are available for specific environments.
