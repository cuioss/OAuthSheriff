...
_See xref:../../doc/specification/well-known.adoc[OIDC Discovery Specification Details]_

When your application uses the `HttpWellKnownResolver` to discover OIDC provider details, you'll want to test this integration without making actual network calls. The `de.cuioss.sheriff.oauth.core.test.dispatcher.WellKnownDispatcher` utility class is provided for this purpose.
...
class MyServiceUsingWellKnownTest {
    ...
    void testSuccessfulOidcDiscoveryAndUsage(URIBuilder uriBuilder) throws Exception {
        ...
        // Example: Configure HttpJwksLoaderConfig using the well-known URL directly
        HttpJwksLoaderConfig jwksConfig = HttpJwksLoaderConfig.builder()
            .wellKnownUrl(wellKnownUrl.toString())
            .build();

        // Note: The JWKS URI will be discovered automatically from the well-known endpoint
    ...
    void testOidcDiscoveryError(URIBuilder uriBuilder) throws Exception {
        ...
        WellKnownConfig wellKnownConfig = WellKnownConfig.builder()
            ...
            .parserConfig(parserConfig)
            .build();
        assertThrows(WellKnownDiscoveryException.class, 
            () -> new HttpWellKnownResolver(wellKnownConfig));
